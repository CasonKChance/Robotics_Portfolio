...
void Robot::setVelocityCommand(const VelocityCommand& command) {
    ...
    setActualVelocity(velocityCommand_);
}
...
void Robot::setActualVelocity(const VelocityCommand& command) {
    // Set the actual velocity of the robot by clamping the command to the robot's maximum velocities
    actualVelocity_.linearVelocity = std::clamp(command.linearVelocity, -maximumLinearVelocity_, maximumLinearVelocity_);
    actualVelocity_.angularVelocity = std::clamp(command.angularVelocity, -maximumAngularVelocity_, maximumAngularVelocity_);
}
